...
//--- wavelets start   ---//
double ModelnData::bf_uni(real inv_va, real inv_vd, int ni, int ns, gsl_matrix * gXX, gsl_vector * gph)
{
	int col = 1 + m_df*ns; 
...
//--- wavelets_v1.3 start ---//
void ModelnData::bf_uni(real inv_va, real inv_vd, int ni, int ns, gsl_matrix * gXX, gsl_vector * gph, double * out)
{
	int col = 1 + m_df*ns; 
	...
	//--- wavelets_v1.3 start ---// 
	gsl_vector * gOmega2col = gsl_vector_alloc(col); // the second column of Omega
	gsl_vector * vec01 = gsl_vector_alloc(col);
	gsl_vector_set(vec01, 0, 0);
	gsl_vector_set(vec01, 1, 1);
	//--- wavelets_v1.3 end ---// 
	...
	gsl_vector * bf = gsl_vector_alloc(repeat); 
	//--- wavelets_v1.3 start ---// 
	gsl_vector * beta_mean = gsl_vector_alloc(repeat); 
	gsl_vector * beta_var = gsl_vector_alloc(repeat); 
	//--- wavelets_v1.3 end ---//
	...
	for (unsigned p = 0; p < vsa.size(); p++)
	{
		...
		// res = B estimate
		//--- wavelets_v1.3 start ---// 
		gsl_vector_set(beta_mean, p, gsl_vector_get(res, 1));
		gsl_linalg_cholesky_solve(ginvOmega, vec01, gOmega2col); //gOmega[,2] = omega %*% I[,2]; 
		//--- wavelets_v1.3 end ---// 
		...
		//--- wavelets_v1.3 start ---//
		double var_com = gsl_vector_get(gOmega2col, 1)*(yy - bob)/(double)(ni-2);
		gsl_vector_set(beta_var, p, var_com);
		//--- wavelets_v1.3 end ---//
	...
	gsl_vector * bf_list = gsl_vector_alloc(repeat);
	double beta_mean_f;
	double beta_var_f;
	double maxbf = gsl_vector_max(bf); 
	gsl_vector_add_constant(bf, -maxbf); 
	double factor = 0; 
	for (int i = 0; i < repeat; i++){
	  gsl_vector_set(bf_list, i, exp(gsl_vector_get(bf, i)));
	  factor += gsl_vector_get(bf_list, i); 
	}


	gsl_vector_scale(bf_list, 1/factor);
	gsl_vector_mul(beta_mean, bf_list);
	gsl_vector_mul(bf_list, bf_list);
	gsl_vector_mul(beta_var, bf_list);
	...
	beta_var_f = 0;
	for (int i = 0; i < repeat; i++){
	  beta_mean_f += gsl_vector_get(beta_mean, i);
	  beta_var_f += gsl_vector_get(beta_var, i);
	}
	...
	//cout << "logbf: " << logbf << endl;
	//cout << "beta_mean: " << beta_mean_f << endl;
	//cout << "beta_var: " << beta_var_f << endl;
	//--- wavelets_v1.3 end ---//
	...
	//--- wavelets_v1.3 start ---//
	gsl_vector_free(gOmega2col);
	gsl_vector_free(vec01);
	gsl_vector_free(beta_mean);
	gsl_vector_free(beta_var);
	gsl_vector_free(bf_list);
	//--- wavelets_v1.3 end ---//
...
void ModelnData::single_snp_functional_phenotype(int mode, int numPerm, int nullcheck)
{
  ...
  if(mode == 2){
    ...
    gsl_vector * eachph = gsl_vector_alloc(nPH); 
    vector<int> ix_perm;
    ix_perm.resize(0);
    for(int i = 0; i < nCohort; i++){
      ix_perm.push_back(i);
    ...
    vector<double> pval; // pvalue for each SNP
    pval.resize(nLoci);

    
    vector<int> numStop; // permutation step we stop
    numStop.resize(nLoci);


    vector<int> doneP; // indicator if each SNP needs more permutation or not
    doneP.resize(0);
    for(int i = 0; i < nLoci; i++){
      doneP.push_back(0);
    }   
    ...
    sfn_pval.append(".fph.pval.txt");
    outfile_pval.open(sfn_pval.c_str(), ios::out);
    if(!outfile_pval.is_open()) {
      cout << "can't open file ... " << endl;  
      exit(0); 
    }

    for(int g =0; g < nLoci; g++){
      outfile_pval << vsRsnum.at(g) << " "; 
    }
    outfile_pval << endl;
    ...
    for(int g =0; g < nLoci; g++){
      ...
      for (int i = 0; i < nIndiv; i++){
	if(pIndiv[i]->GetisPanel()) continue; 
	gsl_matrix_set(gMat_all, ni, g, pIndiv[i]->get_snpmgt(g));
	ni++; 
      }	
    }
    ...
    for(int eachP = 0; eachP < numPerm; eachP++){
     ...
     if(passCount == nLoci){
      ...
      // Let's permute phenotype data!!
      random_shuffle(ix_perm.begin(), ix_perm.end());
      for(int i = 0; i < nCohort; i++){
	gsl_matrix_get_row(eachph, phMat, ix_perm[i]);
	gsl_matrix_set_row(phMat_perm, i, eachph);
      }    
	...
	gsl_matrix_get_col(ph, gMat_all, g);
	gsl_matrix_set_col(gMat, 1, ph);
	
	for(int i =0; i < nPH_use; i++){
	  int p = use_pheno_t[i];
	  gsl_matrix_get_col(ph, phMat_perm, p);
	  logbf = bf_uni(inv_va, inv_vd, nCohort, 1, gMat, ph); 
	  gsl_vector_set(logBFs, p, logbf);
	}
	...
	if(logLR >= logLR_list[g]){
	  ...
	  if(numExt[g] == numSig){
	    ...
	    real rand = gsl_rng_uniform(gsl_r); 
	    pval[g] = st_val + rand*(en_val - st_val);
	    passCount++;
	    doneP[g] = 1;
	  }
	}
    ...
    gsl_vector_free(eachph);
    gsl_matrix_free(gMat_all);
  ...
  if(mode == 3){
    ...
    gsl_vector * logLR_vec = gsl_vector_alloc(nLoci);
    for(int g =0; g < nLoci; g++){
      gsl_vector_set(logLR_vec, g, logLR_list[g]);
    }
    ...
    outfile_pval << vsRsnum.at(max_ix_logLR) << " ";
    outfile_pval << endl;
	...
	if(max_logLR_perm  == 0){
	  ...
	  if(numExt == numSig){
	    numStop = eachP + 1;
	    double st_val = (double)(numSig + 1)/(double)(numStop + 2);
	    double en_val = (double)(numSig + 1)/(double)(numStop + 1);
	    real rand = gsl_rng_uniform(gsl_r); 
	    pval = st_val + rand*(en_val - st_val);
	    doneP = 1;
	  }
      }
    }
    ...
    if(doneP == 0){
      outfile_pval << numPerm;
      pval = (double)(numExt+1)/denNumPerm;
    }else{
      outfile_pval << numStop;
    }
    outfile_pval << endl;
...
//		}
	...
	for (int m = 0; m < np; m++){
	  ...
	  char buf[100]; 
	  if(avBFs[m] < 1e-5) 
	    sprintf(buf, "%.5f ", avBFs[m]); 
	  else 
	    sprintf(buf, "%+.5f ", avBFs[m]); 
	  outfile_BFs << buf; 

	  if(mbf[posiAll][m] < 1e-5) 
	    sprintf(buf, "%.5f ", mbf[posiAll][m]); 
	  else 
	    sprintf(buf, "%+.5f ", mbf[posiAll][m]); 
	  outfile_BFs << buf; 
	  
	  for(int i = 0; i < nd; i++){
	    if(mbf[uniposi[i]][m] < 1e-5) 
	      sprintf(buf, "%.5f ", mbf[uniposi[i]][m]); 
	    else 
	      sprintf(buf, "%+.5f ", mbf[uniposi[i]][m]); 
	    outfile_BFs << buf; 
	  }
