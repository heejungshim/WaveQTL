...
private:
	...
	//--- wavelets start ---//
	vector<int>  group_start; // to save starting positions for groups of phenotypes provided by user 
	vector<int>  use_pheno;   // to save an indicator (1: yes 0: no) to indiciate if the phenotype in that position should be used in the analysis or not (assign 1 to BF). 
	vector<string> group_start_n; // file name
	vector<string> use_pheno_n;   // file name
	vector<int> use_pheno_t;  // to save index for phenotypes to be used
	vector<int> use_pheno_f;  // to save index for phenotypes to be used
	//--- wavelets end ---//
...
public:
   ...
   //--- wavelets_v1.3 end ---//
   void read_extra_information_for_functional_phenotype();
   inline void SetGroupStartN(string s) {group_start_n.push_back(s);}
   inline void SetUsePhenoN(string s) {use_pheno_n.push_back(s);}
   //--- wavelets end   ---//
};    
